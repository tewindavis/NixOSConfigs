...
*   **No Lock Mid-Movie:** the dim, lock and suspend timers all hold off while audio is actually playing, and re-arm within half a minute of pausing — no need to remember the manual idle inhibitor.
*   **Battery Warnings:** a notification at 20% and a critical one at 10% while discharging — once each per discharge, rearmed when you plug back in.
