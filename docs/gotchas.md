...
- **A wedged dock hub can stop fwupd starting at all.** With the CalDigit
  ...
  A third: don't install it with `systemd.tmpfiles`. The directory lives
  inside `/var/lib/fwupd`, which belongs to `fwupd-refresh`, and
  ...
  canonicalization", its symlink-attack guard. It declines *without* failing
  the unit, so the only symptom is a missing file and a single journal line;
  the config looks correct and the fix is simply absent. A oneshot unit
  ordered `Before=fwupd.service` does it instead, and fails loudly if it
  ever can't.
